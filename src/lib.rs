...
fn init_opts() -> Options {
    ...
    options.optflagopt(
        "d",
        "depth",
        "show directories up to depth N (def 1)",
        "DEPTH",
    );
    options.optflagopt(
        "a",
        "aggr",
        "aggregate smaller than N B/KiB/MiB/GiB (def 1M)",
        "N[KMG]",
    );
...
impl<'a> Entry<'a> {
    fn new(path: &Path, cfg: &'a Config, depth: u8) -> Entry<'a> {
        ...
        let color = if !cfg.ascii_flag {
            color_from_path(path, &cfg.color_dict)
        } else {
            None
        };
...
fn fmt_size_str(bytes: u64, flag: bool) -> String {
    ...
    if bytes < 1024 || flag {
        ...
        format!("{:.2} MiB", b / (1024u32.pow(2) as f32))
    } else if bytes < 1024u64.pow(4) {
        format!("{:.2} GiB", b / (1024u32.pow(3) as f32))
    } else {
        format!("{:.2} TiB", b / (1024u64.pow(4) as f32))
    }
}
...
mod tests {
    ...
    fn parse_ls_colors() {
        ...
        assert_eq!(
            "mp3code",
            color_from_path(Path::new("test.mp3"), &dict).unwrap()
        );
        assert_eq!(
            "tarcode",
            color_from_path(Path::new("test.tar"), &dict).unwrap()
        );
    }
